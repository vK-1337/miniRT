...
/*   Updated: 2024/07/29 17:58:36 by vda-conc         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_tuple	ft_normal_at(t_comps object, t_tuple world_point)
{
	...
	if (object.type == SPHERE)
	{
		...
		object_point = ft_mult_matrix_tuple(ft_inversion(matrix, 4),
				&world_point, FIRST);
		object_normal = ft_dif_tuple(object_point, ft_init_tuple_reg(0, 0, 0,
					1));
	}
	...
	else if (object.type == CYLINDER)
	{
		...
		if (dist < pow(object.cylinder->radius, 2)
			...
			object_normal = ft_init_tuple_reg(object_point.x, 0, object_point.z,
					0);
	}
	else if (object.type == CONE)
	{
		...
		{
			object_normal = ft_init_tuple_reg(object_point.x, y, object_point.z,
					0);
			if (object_point.y > 0)
				object_normal.y = -y;
		}
